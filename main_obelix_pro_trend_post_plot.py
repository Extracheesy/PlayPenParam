...
def strategy_6_price_action_series(data, window=20):
    """
    ...
    Parameters:
        ...
        window (int): Lookback period for highest and lowest values.

    Returns:
        pd.Series: Signal series with 1 for buy, -1 for sell, and 0 otherwise.
    """
    high = data.get('High')
    low = data.get('Low')
    ...
    highest_window = high.rolling(window=window).max()
    lowest_window = low.rolling(window=window).min()

    buy_signal = close > highest_window.shift(1)
    sell_signal = close < lowest_window.shift(1)

    strategy_6_signal = pd.Series(0, index=close.index)
    strategy_6_signal[buy_signal] = 1
    strategy_6_signal[sell_signal] = -1

    return strategy_6_signal
...
class IchimokuZemaStrategy:
    ...
    def calculate_indicators(self):
        ...
        try:
            if self.trend_indicator == "ICHIMOKU_INIT":
        ...
        if self.trend_indicator == "PRICE_ACTION":
            price_action_signals = strategy_6_price_action_series(self.high_tf_data, window=20)

        # Ensure all variables exist before assigning them to the dictionary
        zlema_buy_adj = locals().get("zlema_buy_adj", None)
        zlema_sell_adj = locals().get("zlema_sell_adj", None)
        zlma_buy_adj = locals().get("zlma_buy_adj", None)
        zlma_sell_adj = locals().get("zlma_sell_adj", None)
        tema_buy_adj = locals().get("tema_buy_adj", None)
        tema_sell_adj = locals().get("tema_sell_adj", None)
        dema_buy_adj = locals().get("dema_buy_adj", None)
        dema_sell_adj = locals().get("dema_sell_adj", None)
        alma_buy_adj = locals().get("alma_buy_adj", None)
        alma_sell_adj = locals().get("alma_sell_adj", None)
        kama_buy_adj = locals().get("kama_buy_adj", None)
        kama_sell_adj = locals().get("kama_sell_adj", None)
        hma_buy_adj = locals().get("hma_buy_adj", None)
        hma_sell_adj = locals().get("hma_sell_adj", None)
        price_action_signals = locals().get("price_action_signals", None)

        # Initialize self attributes if they don't exist
        self.ichimoku_valid = getattr(self, "ichimoku_valid", None)
        self.trend_pulse = getattr(self, "trend_pulse", None)
        self.bear_trend_pulse = getattr(self, "bear_trend_pulse", None)
        ...
        if self.trend_indicator == "ICHIMOKU_INIT":
            ...
            if self.ma_type in adj_map:
                try:
                    ...
                    exit(444)
        elif self.trend_indicator == "PRICE_ACTION":
            adj_map = {
                "ZLEMA": ("zlema_buy_adj", "zlema_sell_adj"),
                "ZLMA": ("zlma_buy_adj", "zlma_sell_adj"),
                "TEMA": ("tema_buy_adj", "tema_sell_adj"),
            ...
            if self.ma_type in adj_map:
                try:
                    ...
                    # Generate signals for long
                    buy_signal_long = (
                        (self.dct_indicators['price_action_signals'] == 1) &
                        (data_close < buy_adj_aligned)
                    ).astype(int)
                    ...
                    # Generate signals for short
                    buy_signal_short = (
                        (self.dct_indicators['price_action_signals'] == 0) &
                        (data_close > buy_adj_aligned)
                    ).astype(int)
                    sell_signal_short = (data_close < sell_adj_aligned).astype(int)
    ...
    def get_results(self):
        ...
        for symbol in self.data.columns:
            ...
            stats['Type'] = "vbt_pro"
            stats["id"] = self.id
            stats["TREND_INDICATOR"] = self.trend_indicator
            stats['Symbol'] = symbol
            ...
            if False:
                ...
                directory = "./output_plot"
                combined_path = os.path.join(input_directory, directory)
                ensure_directory_exists(combined_path)
                filename = str(stats["id"]) + "_" + stats['Symbol'] + "_" + stats['Timeframe'] + "_" + stats['MA_Type'] + ".html"
...
def parallel_execute(scored_results_path, data, batch_results_folder_path):
    ...
    if not "TREND_INDICATOR" in df.columns:
